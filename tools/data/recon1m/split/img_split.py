...
def add_parser(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='json file defining train/val/test splits')
    ...
    parser.add_argument(
        ...
        default=None,
        help='which splits to process: train val test (can be multiple)')
...
def parse_args():
    ...
    assert args.dataset_split_json is None or osp.isfile(
        args.dataset_split_json), 'dataset_split.json not found'
    if args.split is not None:
        assert args.dataset_split_json is not None, \
            '--split requires --dataset-split-json'
    return args
...
def load_dataset_split(split_json, splits):
    """
    Args:
        split_json (str): path to dataset_split.json
        splits (list[str]): ['train'], ['val'], ['train','val'], etc.

    Returns:
        set[str]: image ids to keep
    """
    ...
    for s in splits:
        if s not in data:
            raise KeyError(f'{s} not found in dataset_split.json')
        selected_ids.update(data[s])
...
def main():
    ...
    if args.dataset_split_json is not None and args.split is not None:
        selected_ids = load_dataset_split(
            args.dataset_split_json, args.split)

        filtered = [
            (info, img_dir)
            for info, img_dir in zip(infos, img_dirs)
            if info['id'] in selected_ids
        ]

        infos, img_dirs = zip(*filtered) if filtered else ([], [])
        infos, img_dirs = list(infos), list(img_dirs)

        print(f'Using splits {args.split}, total images: {len(infos)}')
